#!/usr/bin/env python3
"""E2E validation for runtime NPC reconciliation on continual-harness init state."""
...
def test_continual_harness_init_birch_lab_runtime_reconciliation(tmp_path):
    project_root = Path(__file__).resolve().parents[1]
    rom_path = project_root / "Emerald-GBAdvance" / "rom.gba"
    init_zip = project_root / "Emerald-GBAdvance" / "continual-harness_init.zip"

    if not rom_path.exists():
        pytest.skip("ROM not available for e2e test")
    if not init_zip.exists():
        pytest.skip("continual-harness init zip not available for e2e test")
    ...
    with zipfile.ZipFile(init_zip, "r") as archive:
        ...
        assert checkpoint_members, "Expected checkpoint.state in continual-harness_init.zip"
    ...
    try:
        ...
        assert runtime_npcs, "Expected runtime NPCs in Birch's Lab continual-harness init state"
        ...
        # ContinualHarness init should at least keep live lab NPCs and remove starter balls.
